...
class SendGridEmailBackend(BaseEmailBackend):
    """Django email backend that sends via SendGrid v3 Web API (HTTPS).

    Uses the requests library (already a dependency) instead of SMTP,
    avoiding Render free tier's SMTP port restrictions.

    Requires SENDGRID_API_KEY env var to be set.
    ...
    def send_messages(self, email_messages):
        """Send one or more EmailMessage objects via SendGrid API."""
        if not self.api_key:
    ...
    def _send_one(self, message):
        """Send a single EmailMessage via SendGrid v3 API."""
        ...
        if resp.status_code not in (200, 201, 202):
            ...
            logger.error(
                ...
                resp.status_code, resp.reason, body,
            )
